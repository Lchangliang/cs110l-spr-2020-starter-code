    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                ...
                grid.set(i+1, j+1, 
                    max(grid.get(i+1, j).unwrap(), grid.get(i, j+1).unwrap())).unwrap();
            }
        }
    }
    return grid;
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("The filename1 is invalid");
    let lines2 = read_file_lines(filename2).expect("The filename2 is invalid");
